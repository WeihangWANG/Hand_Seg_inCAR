...
def pre_filter(img):
    ...
    if img.dtype == np.uint8:
        ...
        ret, img_f = cv2.threshold(img_f, 0, 255, cv2.THRESH_BINARY)
    ...
    if max_contour is not None and len(max_contour) > 0:
        ...
        img_tmp = cv2.cvtColor(img_src_f, cv2.COLOR_GRAY2BGR)
        # cv2.drawContours(img_tmp, [max_contour], 0, (0,0,255), 3)
        # cv2.imshow("prefilter",img_tmp)
        # cv2.waitKey(0)
