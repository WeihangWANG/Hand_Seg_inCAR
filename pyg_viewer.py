...
# pygame窗口显示管理
class pyg_viewer_c:
    def __init__(self,pan_wid=320,pan_hgt=240,pan=(1,1),name='pygame window'):
        pygame.init()
    ...
    # 在给定的板块上矩形
    def draw_rect(self,start_h,start_v,end_h,end_v,pan_id=(0,0),color=pygame.color.THECOLORS["white"],line_wid=8):
        if not (pan_id in self.pan_id_pos): return False
        s=self.pan_surf[pan_id]
        pygame.draw.rect(s,color,(start_h,start_v,end_h,end_v),line_wid)
        return True
    ...
    # 轮询用户事件
    def poll_evt(self):
        return pygame.event.get()   # User did something
